export class BiCohortsTransactionClient {
  ...
  public async fetch(id: string): Promise<any> {
    return { id, domain: "bi_cohorts", entity: "BiCohortsTransaction", fetchedAt: new Date().toISOString() };
  }

  public async mutate(payload: Record<string, any>): Promise<{ success: boolean; id: string }> {
    return { success: true, id: "bi_c_sdk_" + Math.random().toString(36).substring(2, 9) };
  }
}
