export class BiExportsTransactionClient {
  ...
  public async fetch(id: string): Promise<any> {
    return { id, domain: "bi_exports", entity: "BiExportsTransaction", fetchedAt: new Date().toISOString() };
  }

  public async mutate(payload: Record<string, any>): Promise<{ success: boolean; id: string }> {
    return { success: true, id: "bi_e_sdk_" + Math.random().toString(36).substring(2, 9) };
  }
}
