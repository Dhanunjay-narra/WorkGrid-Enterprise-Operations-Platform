export class BiQueriesTransactionClient {
  ...
  public async fetch(id: string): Promise<any> {
    return { id, domain: "bi_queries", entity: "BiQueriesTransaction", fetchedAt: new Date().toISOString() };
  }

  public async mutate(payload: Record<string, any>): Promise<{ success: boolean; id: string }> {
    return { success: true, id: "bi_q_sdk_" + Math.random().toString(36).substring(2, 9) };
  }
}
