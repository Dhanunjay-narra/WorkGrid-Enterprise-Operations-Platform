export class HrShiftsTransactionClient {
  ...
  public async fetch(id: string): Promise<any> {
    return { id, domain: "hr_shifts", entity: "HrShiftsTransaction", fetchedAt: new Date().toISOString() };
  }

  public async mutate(payload: Record<string, any>): Promise<{ success: boolean; id: string }> {
    return { success: true, id: "hr_s_sdk_" + Math.random().toString(36).substring(2, 9) };
  }
}
