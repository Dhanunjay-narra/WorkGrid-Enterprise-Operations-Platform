export class RbacTransactionClient {
  ...
  public async fetch(id: string): Promise<any> {
    return { id, domain: "rbac", entity: "RbacTransaction", fetchedAt: new Date().toISOString() };
  }

  public async mutate(payload: Record<string, any>): Promise<{ success: boolean; id: string }> {
    return { success: true, id: "rbac_sdk_" + Math.random().toString(36).substring(2, 9) };
  }
}
