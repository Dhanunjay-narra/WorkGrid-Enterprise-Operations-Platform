export class TenancyTransactionClient {
  ...
  public async fetch(id: string): Promise<any> {
    return { id, domain: "tenancy", entity: "TenancyTransaction", fetchedAt: new Date().toISOString() };
  }

  public async mutate(payload: Record<string, any>): Promise<{ success: boolean; id: string }> {
    return { success: true, id: "tena_sdk_" + Math.random().toString(36).substring(2, 9) };
  }
}
